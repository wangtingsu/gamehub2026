  ...
  return (
    <section className="mb-12">
      <div className="flex justify-between items-center mb-6">
        <div>
          <Title level={2} className="flex items-center gap-2 !text-white !mb-1">
            ...
            {t('hotDiscussions', 'Hot Discussions')}
          </Title>
          <Paragraph className="text-gray-400 !mb-0">
            {t('hotDiscussionsDesc', 'The most active discussions in the community')}
          </Paragraph>
        </div>
        <Button type="link" onClick={() => navigate(`/${currentLang}/community-forum`)}>
          {t('viewAll', 'View All')} <ArrowRightOutlined />
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 text-gray-400">{t('noDiscussions', 'No hot discussions yet')}</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-gray-500 text-xs">
                      <span className="flex items-center gap-1">
                        ...
                        {post.likes || 0} {t('heat', 'Heat')}
                      </span>
                      <span className="flex items-center gap-1">
                        <MessageOutlined />
                        {post.replyCount || post.commentCount || 0} {t('replies', 'Replies')}
                      </span>
                      <span className="text-gray-600 truncate ml-auto">
                        {post.authorName || post.author || t('common.anonymous', 'Anonymous')}
                      </span>
                    </div>
                    {post.category && (
                      <div className="mt-2">
